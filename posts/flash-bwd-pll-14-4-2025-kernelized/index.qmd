...
In the graph below have mentioned `SCC` several times, which stands for `Strongly Connected Component`, it occurs in a graph if all the nodes of a sub-graph are accessible from every other node within that subgraph. In such case parallelization is not possible.
...
grid.sync()
```
Wondering where is the code analysis for backward pass, that's coming next [click here](/posts/flash-bwd-pll-14-4-2025-kernelized/index.qmd).
